...
class Svc(object):
    def __init__(self,
        ...
        """
        SVC main class for inference
        """
        self.total_steps = total_steps
        self.dev = torch.device("cuda" if torch.cuda.is_available() else "cpu")
        self.diffusion_model, self.vocoder, self.diffusion_args = load_model_vocoder(
            diffusion_model_path, self.dev, config_path=diffusion_config_path, total_steps=self.total_steps)
    ...
    def load_speaker_embedding(self, speaker_name, random_select=True):
        """Load .spk.npy embedding from dataset path"""
        ...
        import random
        selected_file = random.choice(spk_files) if random_select else spk_files[0]
    ...
    def infer(self, speaker, tran, raw_path, spk_emb=None):  # ✅ modified
        """Inference core: perform voice conversion"""
        ...
        # ✅ New: allow direct numpy speaker embedding
        if spk_emb is not None:
            spk_embd = torch.from_numpy(spk_emb).float().unsqueeze(0).to(self.dev)
            print(f"🎯 Using direct speaker embedding, shape={spk_embd.shape}")
        else:
        ...
        with torch.no_grad():
            ...
            audio_mel, attention_gate = self.diffusion_model(
                c, f0, vol, spk_embd=spk_embd, gt_spec=None, infer=True)
    ...
    def slice_inference(self,
                        ...
                        clip_seconds=0,
                        spk=None,
                        spk_emb=None):  # ✅ added parameter
        """Perform sliced inference with optional direct speaker embedding"""
        ...
        for (slice_tag, data) in audio_data:
            ...
            for k, dat in enumerate(datas):
                ...
                # ✅ pass spk_emb downstream
                out_audio, out_sr, out_frame = self.infer(spk, tran, raw_path, spk_emb=spk_emb)
